...
#include "CD_Core.h"          // CD_API
#include "CD_MetaFunctions.h" // is_all_same
...
namespace CD
{
    ...
    template <typename T, size_t SZ>
    struct CD_API Vector : private VectorData<T, SZ>
    {
    protected:
    ...
    template <typename T, size_t SZ> CD::ostream& operator<<(CD::ostream& stream, Vector<T, SZ> const& vector);
    template <typename T, size_t SZ> CD::istream& operator>>(CD::istream& stream, Vector<T, SZ>&       vector);
