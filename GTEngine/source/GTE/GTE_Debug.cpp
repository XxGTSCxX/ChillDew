...
namespace GTE
{
    ...
    void Debug::EnableDeveloperConsole()
    {
        if (!_developer_console_is_visible)
        {
            ...
            HANDLE hStdout = CreateFile(GTE_STRING("CONOUT$"), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
            HANDLE hStdin  = CreateFile(GTE_STRING("CONIN$" ), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
